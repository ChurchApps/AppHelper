...
- HelpIcon, ImageEditor, InputBox, Loading, PageHeader, QuestionEdit, SmallButton, SupportModal

#### Wrapper Components
- NavItem, AppList, ChurchList, UserMenu, NewPrivateMessage, PrivateMessageDetails
...
|-----------|-------|----------|------------|----------------|
| Loading | ✅ (43 files) | ✅ (40 files) | ✅ | Used by: many async components |
...
| PageHeader | ❌ | ❌ | ❌ | Modern page header component (new) |
...
| UserMenu | ✅ (1 file) | ❌ | ❌ | Used by: Header components |
...
### @churchapps/apphelper (Core Package)
- **B1App**: 32/48 components (66.7%) - Heavy usage across all component categories
- **ChumsApp**: 22/48 components (45.8%) - Comprehensive usage of core components
- **LessonsApp**: 17/48 components (35.4%) - Focused on core UI components
...
- **Package breakdown**: Core (66.7%), Donations (61.5%), Login (30%), Markdown (75%)
...
- **Package breakdown**: Core (45.8%), Donations (76.9%), Login (30%), Markdown (50%)
...
- **Notable**: Extensive notes integration, comprehensive donation features, migrating from SiteWrapper to SiteHeader
...
- **Package breakdown**: Core (35.4%), Donations (0%), Login (30%), Markdown (50%)
...
3. **New components**: PageHeader added for modern page-level headers
4. **Underutilized components**: 
   - Wrapper components (AppList, ChurchList) have limited adoption
   - createEmotionCache helper is completely unused
5. **Breaking change impact**: Focus testing on core UI components (Loading, InputBox, ErrorMessages) and helpers (ApiHelper, UserHelper, ArrayHelper)
...
- **ChumsApp**: Most comprehensive integration (45.8% core, 76.9% donations) - ideal reference implementation
- **B1App**: Strong adoption (66.7% core, 61.5% donations) with specialized focus areas
- **LessonsApp**: Foundational usage (35.4% core) with significant growth opportunities
...
- Removed SiteWrapper in favor of modular SiteHeader + PageHeader architecture
- Added PageHeader component for modern page-level headers with statistics support

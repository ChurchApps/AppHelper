...
import { ApiHelper, DateHelper, UniqueIdHelper, CurrencyHelper, AppearanceInterface } from "../helpers";
...
interface Props { personId: string, appName?: string, church?: ChurchInterface, appearance?: AppearanceInterface }
...
export const DonationPage: React.FC<Props> = (props) => {
  ...
  const getPaymentMethodComponents = () => {
    if (!paymentMethods) return <Loading />;
      ...
      <>
        <DonationForm person={person} customerId={customerId} paymentMethods={paymentMethods} stripePromise={stripePromise} donationSuccess={handleDataUpdate} church={props?.church} appearance={props?.appearance} />

...
export class StreamingServiceHelper {
  ...
  static currentConfig: StreamConfigInterface | null;
  static timer: NodeJS.Timeout;

  static checkService() {
    let cs: StreamingServiceExtendedInterface | null = null;
    if (StreamingServiceHelper.currentConfig !== undefined && StreamingServiceHelper.currentConfig !== null) {
      cs = StreamingServiceHelper.determineCurrentService(StreamingServiceHelper.currentConfig.services || []);
    }
    if (JSON.stringify(cs) !== JSON.stringify(StreamingServiceHelper.currentService)) {
      StreamingServiceHelper.currentService = cs;
      if (StreamingServiceHelper.currentServiceChangedCallback !== undefined) {
        StreamingServiceHelper.currentServiceChangedCallback(cs);
      }
    }
  }
      ...
      for (let i = 0; i < services.length; i++) {
        ...
        if (s.localChatEnd !== undefined && s.localEndTime !== undefined) {
          if (currentTime <= s.localChatEnd) {
            if (result == null || (result.localEndTime === undefined || s.localEndTime < result.localEndTime)) {
              result = s;
            }
          }
        }

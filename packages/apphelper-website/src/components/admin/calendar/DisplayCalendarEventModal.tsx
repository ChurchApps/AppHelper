...
export function DisplayCalendarEventModal(props: Props) {
  ...
  return (
    <Dialog open={true} onClose={props.onDone} fullWidth scroll="body">
      ...
      <DialogActions>
        <Button variant="text" onClick={props.onDone} data-testid="calendar-event-cancel-button">Cancel</Button>
        {props.event.eventId && props.mode === "edit" && <Button variant="contained" onClick={handleDelete} data-testid="calendar-event-delete-button">Delete</Button>}
      </DialogActions>
    </Dialog>
  );
}

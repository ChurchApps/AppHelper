...
import { ApiHelper, DateHelper, ErrorMessages, EventHelper } from "@churchapps/apphelper";
...
export function EditEventModal(props: Props) {
  ...
  const handleRecurringSave = async (editType:string) => {
    switch (editType){
      ...
      case "future":
        ...
        if (newEvent.start) rrule.options.until = new Date(newEvent.start);
  ...
  const handleDelete = () => {
    ...
    else if (confirm("Are you sure you wish to delete this event?")) ApiHelper.delete("/events/" + event.id, "ContentApi").then(() => { if (props.onDone) props.onDone(); });
  }
      ...
      <Dialog open={true} onClose={props.onDone} fullScreen>
        ...
        <DialogContent>
          <Grid container spacing={1}>
            <Grid size={{ xs: 6 }}>
              <FormGroup>
                <FormControlLabel control={<Checkbox checked={event.allDay} data-testid="all-day-checkbox" />}  label="All Day" name="allDay" onChange={(_e, checked) => { setEvent({...event, allDay:checked}); }} data-testid="all-day-form-control" aria-label="Mark event as all day" />
              </FormGroup>
            </Grid>
            <Grid size={{ xs: 6 }}>
              <FormGroup>
                <FormControlLabel control={<Checkbox checked={(event.recurrenceRule?.length || 0) > 0} data-testid="recurring-checkbox" />}  label="Recurring" name="recurring" onChange={(_e, checked) => { handleToggleRecurring(checked); }} data-testid="recurring-form-control" aria-label="Mark event as recurring" />
              </FormGroup>
            </Grid>
            {getDates()}

            {((event?.recurrenceRule?.length || 0) > 0) && event.start && <RRuleEditor start={event.start} rRule={event.recurrenceRule || ""} onChange={(rRule:string) => { setRRule(rRule); }} /> }

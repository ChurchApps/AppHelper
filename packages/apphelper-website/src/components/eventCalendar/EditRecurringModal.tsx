...
export function EditRecurringModal(props: Props) {
  ...
  return (
    <Dialog open={true} onClose={props.onDone}>
      ...
      <DialogActions>
        <Button onClick={() => { if (props.onDone) props.onDone("none") }} data-testid="edit-recurring-cancel-button">Cancel</Button>
        <Button onClick={() => { if (props.onDone) props.onDone(editType) }} color="primary" data-testid="edit-recurring-save-button">Save</Button>
      </DialogActions>
    </Dialog>
  );
}
